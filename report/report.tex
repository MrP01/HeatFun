...
\begin{document}
  ...
  So we cannot use \chebyshev points as in \Cref{def:chebpoints} for this interpolation method.
  One way to fix this issue is to use \emph{modified \chebyshev points} (\Cref{def:mod-chebpoints}) as in the DCT-II, leveraging half-way points \parencite{CombTrig}.
...
for ($t = 0$; $t \le T$; $t$ += $\Delta t$); do
  ...
  The three example cases \Cref{fig:example1}, \Cref{fig:example1} and \Cref{fig:example3} demonstrate the correct behaviour of the solver, closesly matching those returned by Chebfun's \texttt{pde15s()}.

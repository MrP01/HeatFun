...
\begin{document}
  ...
  The first two methods demonstrate why it is sensible to choose to sample a function in \textit{chebpoints} (\autoref{def:chebpoints}, also confer \autoref{fig:chebpoints}) instead of equispaced points, simply because the mathematics works out nicely in this basis.
  ...
  And indeed, for larger and larger $N$, they will converge to the correct coefficients according to \autoref{thm:integralapprox}.
  ...
  One way to fix this issue is to use \emph{modified chebpoints} (\autoref{def:mod-chebpoints}) as in the DCT-II, leveraging half-way points \parencite{CombTrig}.
  ...
  We compute the remaining coefficients in a for-loop, directly according to \autoref{eq:coefficient-formula}.
  ...
  The next step is to make sure that the newly obtained $u^{(t+\Delta t)}$ fulfills the boundary conditions $u^{(t)}(-1) = l \;\wedge\; u^{(t)}(1) = r$ as well!

  \subsection{Enforcing Boundary Conditions}
  One way of forcing the boundary conditions, at least the first that came to mind when thinking of this issue, is to pin down the two highest-order coefficients in the series representation after the iteration.
...
for ($t = 0$; $t \le T$; $t$ += $\Delta t$); do
  ...
  Perhaps simpler, more compact and including adaptive time-steps, part of the above algorithm is implemented in C++ as
  \begin{minted}{cpp}
  ...
  One more thing is missing though, given the coefficients $\vec{a}^{(T)}$ we also want to evaluate the Chebyshev series $u^{(T)} \in \mathbb{T}_N$ on some $\vec{x} \subset [-1, 1]$!

  \subsection{Evaluation: The Clenshaw Algorithm}
  For this purpose, let us recall the recurrence relation we proved in the very beginning, \autoref{thm:chebrecursion}.
  ...
  From \autoref{thm:chebrecursion} we identify $\alpha(n, x) = 2x$ and $\beta(n, x) = -1$ to arrive at the following special case for Chebyshev series:
  \begin{minted}{cpp}
  ...
  This implementation can even work with vector-valued $\vec{x}$.
  ...
  Given even more time, it would be interesting to compare the three mentioned coefficient computation methods listed in \autoref{subsection:numerical-coeffs} and look at the potential speed-up facilitated by the FFT.

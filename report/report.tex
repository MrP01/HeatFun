...
for ($t = 0$; $t \le T$; $t$ += $\Delta t$); do
  ...
  Including adaptive time-steps, part of the above algorithm is implemented in C++ as
  \begin{minted}{cpp}
  ...
  If even $u_0 \in \mathbb{T}_N$, then the approximation is exact and the error is inherited from Forward-Euler.

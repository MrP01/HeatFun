...
for ($t = 0$; $t \le T$; $t$ += $\Delta t$); do
  ...
  Essentially, the idea is simple: Using the recurrence relation (\autoref{thm:chebrecursion}) to our advantage and tracking the sum of $a_k T_k(x)$ on the way from $k = N-2$ to $k=0$, which is why we call it the backward Clenshaw recurrence.
